...
ここに出る呼び出しは全て Rust・C ABI・.NET・JavaScript バインディングの 4 経路に存在する（綴りは各言語の流儀）。例は Rust で書くが、各バインディングの綴りはそれぞれの README にある。ファームウェアを書く読み手はこの 4 経路のどれも通らない。機器が持つのはファイルではなくレイアウトで、代わりに何を持つかは [spec/embedded.jp.md](./spec/embedded.jp.md) にある。
