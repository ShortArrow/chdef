...
レイアウトは、測られる相手の上限を持てる。いずれも利用側が指定するもので、CSV から読むことはなく、変換が適用することもない。`check_capacity()` が明示的な問い合わせで、**全ての**指摘を返す（両方を超えたレイアウトを 1 つずつ直す羽目にならないように）。
