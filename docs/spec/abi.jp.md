...
実装済み（0.0.6）: 下記の全部 — レイアウト、変換、名前付きビット、Grid、値の記法、Issue を、`chdef-capi` の C ABI と、その上に立つ .NET バインディング経由で。TypeScript は未実装。
...
- **ハンドルは不透明でタグ付き。** 生成時にタグを打ち、解放時に消すので、古いポインタや他所のポインタは参照されずに `CHDEF_ERR_HANDLE` になる。二重解放は安全で、null は無視される。
